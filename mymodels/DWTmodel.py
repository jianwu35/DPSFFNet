...
from 超声心动图.超声心动图.Echotest.echonet.utils.models.EMA.EMA import EMA
from 超声心动图.超声心动图.ISIC.mymodels.DWTconv import WTConv2d
...
class EncoderBottleneck2(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, base_width=64):
        ...
        self.norm1 = nn.BatchNorm2d(width)  # 第一个批量归一化层
        self.conv2 = WTConv2d(width, width, kernel_size=3, stride=2, bias=False)  # 第二个卷积层
        self.norm2 = nn.BatchNorm2d(width)  # 第二个批量归一化层
        self.conv3 = WTConv2d(width, out_channels, kernel_size=1, stride=1, bias=False)  # 第三个卷积层
        self.norm3 = nn.BatchNorm2d(out_channels)  # 第三个批量归一化层
        self.relu = nn.ReLU(inplace=True)  # ReLU激活函数
...
class Encoder1(nn.Module):
    def __init__(self, in_channels, out_channels, head_num, mlp_dim, block_num, patch_dim, img_dim):
        ...
        self.relu = nn.ReLU(inplace=True)  # ReLU激活函数
        self.encoder1 = EncoderBottleneck1(out_channels, out_channels * 2, stride=2)  # 第一个编码器瓶颈层
        self.encoder2 = EncoderBottleneck1(out_channels * 2, out_channels * 4, stride=2)  # 第二个编码器瓶颈层
        self.encoder3 = EncoderBottleneck1(out_channels * 4, out_channels * 8, stride=2)  # 第三个编码器瓶颈层
        self.vit_img_dim = img_dim // patch_dim  # ViT的图像维度
        self.vit = ViT(self.vit_img_dim, out_channels * 8, out_channels * 16,  # ViT模型
                       head_num, mlp_dim, block_num, patch_dim=1, classification=False)
        self.conv2 = nn.Conv2d(out_channels * 16, 512, kernel_size=3, stride=1, padding=1)  # 第四个卷积层
...
class Encoder2(nn.Module):
    def __init__(self,  in_channels, out_channels, head_num, mlp_dim, block_num, patch_dim, img_dim):
        ...
        self.norm1 = nn.BatchNorm2d(out_channels)  # 第一个批量归一化层
        self.relu = nn.ReLU(inplace=True)  # ReLU激活函数
        self.encoder1 = EncoderBottleneck2(out_channels, out_channels * 2, stride=2)  # 第一个编码器瓶颈层
        self.encoder2 = EncoderBottleneck2(out_channels * 2, out_channels * 4, stride=2)  # 第二个编码器瓶颈层
        self.encoder3 = EncoderBottleneck2(out_channels * 4, out_channels * 8, stride=2)  # 第三个编码器瓶颈层
        self.vit_img_dim = img_dim // patch_dim  # ViT的图像维度
        self.vit = ViT(self.vit_img_dim, out_channels * 8, out_channels * 16,  # ViT模型
                       head_num, mlp_dim, block_num, patch_dim=1, classification=False)
        self.conv2 = nn.Conv2d(out_channels * 16, 512, kernel_size=3, stride=1, padding=1)  # 第四个卷积层
...
class Decoder(nn.Module):
    def __init__(self, out_channels, class_num):
        super().__init__()  # 初始化父类
        self.decoder1 = DecoderBottleneck(512, 256)  # 第一个解码器瓶颈层
        self.decoder2 = DecoderBottleneck(512, 128)  # 第二个解码器瓶颈层
        self.decoder3 = DecoderBottleneck(384, 64)  # 第三个解码器瓶颈层
...
class TransUNet(nn.Module):
    def __init__(self, in_channels, out_channels, head_num, mlp_dim, block_num, patch_dim, class_num, img_dim
        ...
        self.aspp = ASPP(1024, atrous_rates=[1, 2, 3, 6])  # 初始化ASPP模块
        self.aspp1 = ASPP(out_channels * 2, atrous_rates=[1, 2, 3, 6])  # 初始化ASPP模块
        self.aspp2 = ASPP(out_channels * 4, atrous_rates=[1, 2, 3, 6])  # 初始化ASPP模块
        self.aspp3 = ASPP(out_channels * 8, atrous_rates=[1, 2, 3, 6])  # 初始化ASPP模块
    ...
    def forward(self, x):
        ...
        x = self.decoder(x0, x1, x2, x3)  # 解码分支  # 解码分支
...
if __name__ == '__main__':
    ...
    head_num = 16  # Transformer 头的数量
    mlp_dim = 256  # 多层感知机（MLP）的维度
    block_num = 25  # Transformer 块的数量
    patch_dim = 16  # 图像被分割成的小块的大小
    class_num = 3  # 类别数，对于二分类或单类别分割问题可能是1
    # 使用这些参数创建 TransUNet 类的实例
    model = TransUNet(in_channels=1, out_channels=3, img_dim=img_dim, head_num=head_num, mlp_dim=mlp_dim,
